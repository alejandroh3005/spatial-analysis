---
title: "Notes on Spatial Data Analysis"
author: |
  | Jon Wakefield and Alejandro Hernandez
...
These notes were originally written by Jon Wakefield PhD. at the University of Washington to be shared with students of his spatial analysis course(s). I, Alejandro Hernandez, have updated grammar of the code and revised some language for clarity. I intend for these revisions to be useful to me and future students.
...
* Now ``sf`` has been replaced, details are [here](https://r-spatial.github.io/sf/)
...
#   st_as_sf() |> 
#   st_make_valid() -> nc1
nc1 <- map("county", "North Carolina", fill = TRUE, plot = FALSE) %>%
  st_as_sf() %>% 
  st_make_valid()
...
We can repeat this example for Washington state.

```{r}
wa1 <- map("county", "Washington", fill = TRUE, plot = FALSE) %>%
  st_as_sf() %>% 
  st_make_valid()
centroids <- st_centroid(st_geometry(wa1)) %>% st_coordinates()
plot(st_geometry(wa1))
text(centroids, labels = 1:nrow(wa1), col = "orange",cex=.5)
```
